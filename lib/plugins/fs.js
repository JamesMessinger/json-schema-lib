...
module.exports = {
  ...
  readFileSync: function readFileSync (args) {
    ...
    return fs.readFileSync(file.url, file.encoding);
  },
  ...
  readFileAsync: function readFileSync (args) {
    ...
    inferFileMetadata(file);
    fs.readFile(file.url, file.encoding, next);
  },
};
...
function inferFileMetadata (file) {
  file.mimeType = mime.lookup(file.url) || null;
  file.encoding = mime.charset(file.mimeType) || null;
}
