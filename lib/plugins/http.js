  ...
  readFileAsync: function readFileSync (args) {
    ...
    req.once('response', function handleResponse (res) {
      ...
      res.on('end', function handleResponseEnd () {
        try {
          var response = decodeResponse(responseChunks, file.encoding);
          next(null, response);
        }
        catch (err) {
          next(err);
...
function setFileMetadata (file, res) {
  ...
  if (header && typeof header === 'string') {
    ...
    file.mimeType = parsed.type;
    file.encoding = parsed.parameters.charset || null;
  }
}

/**
 * Assembles the response from the given chunks, and decodes it if possible.
 *
 * @param {array} chunks - An array of strings or Buffers
 * @param {?string} encoding - The response encoding, if any
 * @returns {string|Buffer}
 */
function decodeResponse (chunks, encoding) {
  if (typeof chunks[0] === 'string') {
    // The response was returned as strings, not Buffers.
    // So just join all the strings.  No need to decode anything.
    return chunks.join('');
  }
  ...
  if (encoding) {
    try {
      ...
      response = Buffer.toString(encoding);
    }
    catch (err) {
      // Unknown encoding, so just return the response as a raw Buffer
    }
  }
