 ...
 * JSON Pointers.
 *
 * @param {object} [config] - The config settings that apply to the schema
 * @param {object[]} [plugins] - The plugins to use for the schema
 * @param {object} [defaultConfig] - The default configuration for any settings that aren't by `config`
 *
 * @class
 */
function Schema (config, plugins, defaultConfig) {
  /**
   * The config settings that apply to this schema.
   *
   * @type {Config}
   */
  this.config = new Config(config, defaultConfig);
   ...
   * @type {PluginHelper}
   */
  this.plugins = new PluginHelper(plugins);
